...
import com.knoldus.daml.bootstrap.{ RouteInstantiator, ServiceInstantiator}
...
object HttpServerApp extends App {
  ...
  try {
    ...
    val services = new ServiceInstantiator(logger)
    val routes = new RouteInstantiator(services,logger).routes
